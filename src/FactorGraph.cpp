...
namespace optimization
{
    ...
    void Optimizer::OptimizeWithGroundNormal(std::vector<Frame> &frame_list, const std::vector<FrameCorrespondence> &frame_corres, double ground_weight, bool fix_begin, bool fix_end)
    {
        ceres::LossFunction *loss_function = new ceres::HuberLoss(0.1);
        ceres::Problem::Options problem_options;
        ceres::Problem problem(problem_options);
        ...
        {
            ceres::CostFunction *cost_function = 
                factor::Plane2PlaneEquationFactor::Create(frame_list[i].ground_plane_normal, 
                    frame_list[0].ground_plane_normal, ground_weight);
            problem.AddResidualBlock(cost_function, loss_function, frame_list[i].pose.data());
        }
        ceres::Solver::Options options;
        options.linear_solver_type = ceres::ITERATIVE_SCHUR;
        options.max_num_iterations = max_num_iterations;

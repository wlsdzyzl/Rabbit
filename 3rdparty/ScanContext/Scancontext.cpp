...
std::vector<ScanTuple> SCManager::detectLoopClosureIDs ( void )
{

    auto curr_key = polarcontext_invkeys_mat_.back(); // current observation (query)
    auto curr_desc = polarcontexts_.back(); // current observation (query)
    std::vector<ScanTuple> results;
    /* 
     * step 1: candidates from ringkey tree_
     */
    if( polarcontext_invkeys_mat_.size() < NUM_EXCLUDE_RECENT + 1)
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        if( candidate_dist < SC_DIST_THRES )
        {
            ...
            results.push_back(std::make_tuple(candidate_indexes[candidate_iter_idx], yaw_diff_rad, candidate_dist));
        }
    }
    t_calc_dist.toc("Distance calc");

    /* 
     * sort from small dist to large dist
     */
...
} // SCManager::detectLoopClosureIDs
